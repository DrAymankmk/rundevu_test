	...
	<div class="row">
		<div class="col-12">
			<div class="page-title-box">
				...
				<h4 class="page-title">{{ __('blog.edit_category') }}: {{ $category->name }}</h4>
			</div>
		</div>
	</div>
							...
							<div class="tab-pane fade {{ $index === 0 ? 'show active' : '' }}"
								...
								<div class="mb-3">
									...
									<textarea class="form-control @error('translations.'.$lang->code.'.description') is-invalid @enderror"
										...
										rows="4"
										dir="{{ $lang->direction }}">{{ old('translations.'.$lang->code.'.description', $translation->description ?? '') }}</textarea>
				...
				<div class="card">
					...
					<div class="card-body">
						...
						@include('components.image-upload', [
							...
							'collection' => 'image',
							'label' => __('blog.image'),
							'existingImage' => $category->getImageUrl('thumb'),
							'model' => $category,
						])
						...
						<div class="d-grid">
							<button type="submit" class="btn btn-primary" {{ $languages->isEmpty() ? 'disabled' : '' }}>
								<i class="mdi mdi-content-save"></i> {{ __('blog.update_category') }}
							</button>
						</div>
					</div>
				</div>
