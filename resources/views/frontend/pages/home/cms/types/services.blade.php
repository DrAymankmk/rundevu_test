...
collect();
$bg = asset('frontend/assets/img/services_bg.jpeg');
	...
	<div class="container">
		<div class="row justify-content-center">
			<div class="col-xl-5">
				<div class="title-area text-center">
					<span
						class="sub-title">{{ $st?->subtitle ?: __('Our Services') }}</span>
					<h2 class="sec-title">{!! $st?->title ?: __('Our Amazing <span
							class="fw-normal">Services</span>') !!}</h2>
					<div class="fs-18 wow fadeInUp cms-services-intro">{!!
						$st?->description ?: e(__('We are equipped with best medical
						services and quality findings.')) !!}</div>
				</div>
			</div>
		</div>
		<div class="service-list-area">
			<div class="row gy-3">
				...
				$icon = $item->getMediaUrl('icons', $locale, asset('frontend/assets/img/icon/' .
				($defaultIcons[$idx % 3] ?? 'service_1_1.svg')), true);
				$simg = $item->getMediaUrl('images', $locale,
				asset('frontend/assets/img/service/' . ($defaultImg[$idx % 3] ??
				'service_2_1.jpg')), true);
				$simgAlt = $item->getMediaAlt('images', $locale, true);
				$itemLink = $getItemLink($item);
				@endphp
				<div class="col-12">
					<div class="service-grid2 wow fadeInUp"
						data-wow-delay="{{ '.' . (2 + $idx) }}s">
						<div class="service-content">
							<div class="box-wrapp">
								<div class="box-icon box-icon--fit">
									<img src="{{ $icon }}"
										alt="{{ $it?->title ?: __('Service') }}"
										loading="lazy"
										decoding="async">
								</div>
								<h3 class="box-title">
									...
									<a href="{{ $itemLink['href'] }}"
										...
										empty($itemLink['rel']))
										rel="{{ $itemLink['rel'] }}"
										@endif>{{ $it?->title ?: __('Service') }}</a>
									@else
									{{ $it?->title ?: __('Service') }}
									@endif
								</h3>
								<div class="box-text">{!! $it?->content
									?: e(__('Quality medical care
				...
				@empty
				<div class="col-12 text-center text-muted">{{ __('No services added yet.') }}
				</div>
				@endforelse
