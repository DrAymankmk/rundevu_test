...
['t' => __('Unlock Your Wellness Journey'), 'c' => __('Make an appointment and take the first step toward better
health.')],
];
...
<div class="position-relative space" id="section-{{ $section->id }}">
	<div class="container">
		...
		<div class="row gy-4 justify-content-center">
			...
			@endphp
			<div class="col-md-6 col-xl-4 feature-card_wrapp wow fadeInUp"
				data-wow-delay="{{ $delay }}">
				<div class="feature-card th-ani">
					<h2 class="box-title">{{ $t }}</h2>
					<div class="box-text">{!! $c !!}</div>
					@if($itemLink)
					<a href="{{ $itemLink['href'] }}" class="th-btn black-border mt-2"
						@if(($itemLink['target'] ?? '_self' )==='_blank' )
						target="_blank" @endif @if(! empty($itemLink['rel']))
						rel="{{ $itemLink['rel'] }}" @endif>
						{{ $itemLink['label'] }}
