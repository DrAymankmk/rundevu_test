        ...
        <div class="content">
            ...
            <div class="d-flex align-items-sm-center flex-sm-row flex-column gap-2 pb-3 mb-3 border-1 border-bottom">
                <div class="flex-grow-1">
                    <h4 class="fw-bold mb-0">{{ trans('main.Patients List') }} <span class="badge badge-soft-primary fw-medium border py-1 px-2 border-primary fs-13 ms-1">{{ trans('main.Total Patients') }} : {{ $users->total() }}</span></h4>
                </div>
...
{{--                    </div>--}}
                    <div class="bg-white border shadow-sm rounded px-1 pb-0 text-center d-flex align-items-center justify-content-center">
                        <a href="{{route('users')}}" class="bg-light rounded p-1 d-flex align-items-center justify-content-center"> <i class="ti ti-list fs-14 text-dark"></i></a>
            ...
            <div class="table-responsive">
                <table class="table datatable table-nowrap">
                    <thead class="text-center {{ app()->getLocale() == 'ar' ? 'text-end' : 'text-start' }}">
                    <tr>
                        <th>{{ trans('main.Patient') }}</th>
                        <th>{{trans('main.Phone')}}</th>
                        <th>{{trans('main.email')}}</th>
                        <th>{{trans('main.expired_date')}}</th>
                        <th>{{ trans('admin.Created Date') }}</th>
                        <th>{{ trans('admin.file_number') }}</th>
                    ...
                    @foreach($users as $user)
                        ...
                        <td>
                            <div class="d-flex align-items-center">
                                ...
                                <a href="{{url('patient-details')}}" class="text-dark fw-semibold">{{ $user->name ?? null }} <span class="text-body fs-13 fw-normal d-block"> {{ \Carbon\Carbon::parse($user->dob)->diff(\Carbon\Carbon::now())->y  }}, @if($user->gender == 1) @lang('admin.male')@else @lang('admin.female') @endif </span>  </a>
                            </div>
                        </td>
                        ...
                        <td>{{ $user->email ?? null }}</td>
                            <td>
                                ...
                                <div>
                                    {{ app()->getLocale() == 'en' ? ($user->package->name_en ?? '-') : ($user->package->name_ar ?? '-') }}
                                </div>
                                <div>
                                    @php
                                        $remainingDays = \Carbon\Carbon::parse($user->expired_date)->diffInDays(now(), false);
                                    @endphp

                                    @if($remainingDays > 0)
                                        {{ $remainingDays }} {{ trans('main.days left') }}
                                    @else
                                        {{ trans('main.Expired') }}
                                    @endif
                                </div>
                            ...
                            <td>{{ $user->file_number ?? null }}</td>
                            <td><span class="badge @if($user->status == 1) badge-soft-success border border-success @else badge-soft-danger border border-danger @endif  px-2 py-1 fs-13 fw-medium">@if($user->status == 1) @lang('admin.Active') @else @lang('admin.Inactive') @endif </span></td>

                            <td>
{{--                                <a href="{{ route('SubSpecialties', $user->id) }}" class="link-reset fs-18 p-1"> <i class="ti ti-eye"></i></a>--}}
                                <a href="{{route('edit-user', $user->id)}}" class="link-reset fs-18 p-1"> <i class="ti ti-pencil"></i></a>
